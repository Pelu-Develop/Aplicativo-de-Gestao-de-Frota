...
interface Rota {
    ...
    saldoPago: boolean;
    dataAdiantamentoPago?: string;
    dataSaldoPago?: string;
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 bg-primary/5 p-4 rounded-xl border border-primary/20 mb-4">
                                            ...
                                            <div className="flex flex-col justify-center gap-2">
                                                <label className="flex items-center gap-2 cursor-pointer group">
                                                    <input type="checkbox" className="rounded border-border text-primary focus:ring-primary size-4" checked={rota.adiantamentoPago} onChange={(e) => {
                                                        updateRota(index, 'adiantamentoPago', e.target.checked);
                                                        if (e.target.checked && !rota.dataAdiantamentoPago) {
                                                            updateRota(index, 'dataAdiantamentoPago', new Date().toISOString().split('T')[0]);
                                                        }
                                                ...
                                                {rota.adiantamentoPago && (
                                                    <input type="date" className="bg-background border border-border rounded-lg px-2 py-1 text-[10px] w-full" value={rota.dataAdiantamentoPago || ''} onChange={(e) => updateRota(index, 'dataAdiantamentoPago', e.target.value)} disabled={formData.status === 'Finalizado'} />
                                                )}
                                                <label className="flex items-center gap-2 cursor-pointer group">
                                                    <input type="checkbox" className="rounded border-border text-emerald-500 focus:ring-emerald-500 size-4" checked={rota.saldoPago} onChange={(e) => {
                                                        updateRota(index, 'saldoPago', e.target.checked);
                                                        if (e.target.checked && !rota.dataSaldoPago) {
                                                            updateRota(index, 'dataSaldoPago', new Date().toISOString().split('T')[0]);
                                                        }
                                                    }} disabled={formData.status === 'Finalizado'} />
                                                ...
                                                {rota.saldoPago && (
                                                    <input type="date" className="bg-background border border-border rounded-lg px-2 py-1 text-[10px] w-full" value={rota.dataSaldoPago || ''} onChange={(e) => updateRota(index, 'dataSaldoPago', e.target.value)} disabled={formData.status === 'Finalizado'} />
                                                )}
                                            </div>
                                            <div className="flex flex-col justify-center gap-2">
                                                <label className="flex items-center gap-2 cursor-pointer group">
